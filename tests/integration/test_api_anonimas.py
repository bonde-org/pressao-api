...
class TestAPIAnonima:
    ...
    def setup_data(self, client, db_session, mock_admin):
        ...
        campanha_resp = client.post(
            "/api/v1/campanhas/", json={"nome": "Campanha Teste Validações"}
        )
    ...
    def test_criar_acao_anonima_com_service_account(
    ...
    def test_criar_acao_com_dados_ativista_por_service_account(
        self, client, db_session, mock_service_account, setup_data
    ):
        ...
        response = client.post(
            ...
            json={
                ...
                "alvo_id": alvo["id"],
                "canal": "email",
                "ativista": {"nome": "João Silva", "email": "joao@email.com"},
            },
        )
        assert response.status_code == 201
        data = response.json()
    ...
    def test_criar_acao_com_usuario_logado(self, client, db_session, mock_user, setup_data):
        ...
        response = client.post(
            "/api/v1/acoes/",
            json={"campanha_id": campanha["id"], "alvo_id": alvo["id"], "canal": "instagram"},
        )
        assert response.status_code == 201
        data = response.json()
        ...
        response2 = client.post(
            ...
            json={
                ...
                "alvo_id": alvo["id"],
                "canal": "email",
                "ativista": {"nome": "Nome Diferente", "email": "outro@email.com"},
            },
        )
    ...
    def test_ativista_anonimo_com_dados_deve_ignorar(
        self, client, db_session, mock_user, setup_data
    ):
